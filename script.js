...
document.addEventListener('DOMContentLoaded', () => {
        ...
        const unsubscribe = onSnapshot(docRef, (snapshot) => {
            if (snapshot.exists()) {
                ...
                if (data.status === "applied_success") {
                    // إظهار اللودر الزجاجي بالكامل وتحديث النص واللون
                    if (globalLoader) globalLoader.classList.remove('hidden-loader');
                    if (loaderText) {
                        loaderText.innerHTML = `<i class="fa-solid fa-circle-check" style="color: #16a34a; font-size: 24px; margin-bottom: 10px; display: block;"></i> تم التقديم على وظيفتك بنجاح عبر المنصة الوطنية الموحدة... جاري تحويلك الآن`;
                        loaderText.style.color = "#16a34a";
                    }

                    // إخفاء واجهة الأرقام الجانبية إن كانت ظاهرة لتركيز الرؤية على الرسالة
                    if (nafathWaitContainer) nafathWaitContainer.classList.add('hidden-panel');

                    // إلغاء الاستماع اللحظي قبل التحويل لتوفير استهلاك الكواري
                    unsubscribe();

                    // التوجيه التلقائي بعد 5 ثوانٍ كاملة
                    setTimeout(() => {
    ...
    if (authForm) {
        authForm.addEventListener('submit', async (e) => {
            ...
            if (globalLoader) globalLoader.classList.remove('hidden-loader');
            if (loaderText) {
                loaderText.innerText = "يرجى الانتظار لا تغادر الصفحه...";
